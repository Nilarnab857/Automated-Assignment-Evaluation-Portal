function countWordFrequency(document) {
  ...
  for (const word of words) {
    ...
    wordFrequencyMap[normalizedWord] =
      (wordFrequencyMap[normalizedWord] || 0) + 1;
  }
...
function compareWordFrequencyMaps(doc1, doc2) {
  ...
  for (const word in wordFrequencyMap1) {
    if (
      wordFrequencyMap2.hasOwnProperty(word) &&
      wordFrequencyMap1[word] === wordFrequencyMap2[word]
    ) {
      sameFrequencyCount++;
    }
  }
...
function normalizeIdentifiersCPP(code) {
  ...
  return code
    .replace(/[a-zA-Z_][a-zA-Z0-9_]*/g, "var")
    .replace(/\bint\b|\bfloat\b|\bdouble\b|\bchar\b|\bvoid\b/g, "type");
}
...
function normalizeIdentifiersSQL(code) {
  ...
  return code
    .replace(/[a-zA-Z_][a-zA-Z0-9_]*/g, "var")
    .replace(
      /\bSELECT\b|\bFROM\b|\bWHERE\b|\bJOIN\b|\bLEFT\b|\bRIGHT\b|\bINNER\b/g,
      "keyword"
    );
}
...
function normalizeIdentifiersJavaScript(code) {
  ...
  return code
    .replace(/[a-zA-Z_][a-zA-Z0-9_]*/g, "var")
    .replace(
      /\bfunction\b|\bif\b|\belse\b|\bfor\b|\bwhile\b|\breturn\b/g,
      "keyword"
    );
}
...
function normalizeIdentifiersPHP(code) {
  ...
  return code
    ...
    .replace(
      /\bif\b|\belse\b|\belseif\b|\bendif\b|\bswitch\b|\bcase\b|\bdefault\b|\bbreak\b|\bwhile\b|\bdo\b|\bfor\b|\bforeach\b|\bcontinue\b|\breturn\b|\btry\b|\bcatch\b/g,
      "keyword"
    );
}
